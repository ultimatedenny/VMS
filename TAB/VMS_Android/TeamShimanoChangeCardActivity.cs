...
{
    [Activity(Label = "Team Shimano Change Card")]
    public class TeamShimanoChangeCardActivity : Activity
    {
        ISharedPreferences session;
        VisitorRepository _visitor = new VisitorRepository();
        private List<Visitor_TS> listVisitor;
        private GridView gv;
        ...
        {
            listVisitor = await _visitor.GetListVisitor(text);
            if (listVisitor.Count > 0)
            {
                gv.Visibility = ViewStates.Visible;
                imageList.Visibility = ViewStates.Invisible;
                gv.Adapter = new TeamShimanoCCAdapter(this, listVisitor);
            }
            else
            {
                gv.Visibility = ViewStates.Invisible;
